...
const upper_images = document.getElementById('upper-slider').querySelectorAll(".slider img");
const bottom_images = document.getElementById('bottom-slider').querySelectorAll(".slider img");
...
function calculateImgWidth() {
    ...
    upper_slider.style.transform = "translateX(" + -(+SIZE_WIDTH) * counter_up + "px)";
    bottom_slider.style.transform = "translateX(" + -(+SIZE_WIDTH) * counter_down + "px)";
}

nextBtn.addEventListener("click", () => {
	if(counter_up + 1 > upper_images.length - 1 || counter_down - 1 < 0 )
		return
...
prevBtn.addEventListener("click", () => {
	if(counter_up - 1 < 0 || counter_down + 1 > bottom_images.length - 1 )
		return
	// !flex direction column
    upper_slider.style.transition = "transform 0.4s ease-in-out";
    bottom_slider.style.transition = "transform 0.4s ease-in-out";
